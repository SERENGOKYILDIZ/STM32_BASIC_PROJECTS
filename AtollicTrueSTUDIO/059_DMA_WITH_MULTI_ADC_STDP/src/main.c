...
uint16_t adc_value[BufferLenght] = {0, 0};
...
void ADC_Config()
{
	...
	ADC_InitStruct.ADC_NbrOfConversion = BufferLenght;                          //->How many ADCs are reading. (2 In our project)
	ADC_Init(ADC1, &ADC_InitStruct);

	ADC_Cmd(ADC1, ENABLE);

	ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_3Cycles);  	//->Rank 1
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 2, ADC_SampleTime_3Cycles);  	//->Rank 2
...
void DMA_Config()
{
	...
	DMA_InitStruct.DMA_BufferSize = BufferLenght;                               //->How many DMA channels. (2 In our project)
